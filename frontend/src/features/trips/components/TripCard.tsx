            ...
            {trip.tags && trip.tags.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-2.5">
                {trip.tags.slice(0, 4).map(tag => (
                  <span
                    key={tag}
                    className="px-2 py-0.5 rounded-full text-xs font-medium bg-brand-50 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400"
                  >
                    #{tag}
                  </span>
                ))}
                {trip.tags.length > 4 && (
                  <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500">
                    +{trip.tags.length - 4}
                  </span>
                )}
              </div>
