    ...
    <>
      ...
      {isLoading && (
        <div className="min-h-screen flex items-center justify-center print:hidden">
          <div className="text-center space-y-3">
            ...
            <p className="text-sm text-gray-400 dark:text-gray-500">Preparing your journal…</p>
          </div>
        </div>
      )}
      ...
      {trip && (
        <div className={isLoading ? 'hidden' : ''}>
          ...
          {/* Screen-only toolbar */}
          <div className="print:hidden sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-3 flex items-center justify-between">
            <Link
              to={`/trips/${id}`}
              className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-200 transition-colors"
            >
          ...
          <div className="max-w-2xl mx-auto px-10 py-12 print:max-w-none print:p-0">
            ...
            {/* Trip title */}
            <h1 className="text-4xl font-bold text-gray-900 dark:text-white leading-tight mb-3 print:text-3xl">
              {trip.title}
            </h1>

            {/* Meta row */}
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-sm text-gray-500 dark:text-gray-400 mb-5">
            ...
            {/* Description */}
            {trip.description && (
              <p className="text-[15px] text-gray-600 dark:text-gray-300 leading-relaxed border-l-2 border-brand-200 pl-4 mb-8 print:mb-6">
                {trip.description}
              </p>
            )}
            ...
            {destinations.length > 0 && (
              <>
                <hr className="border-gray-200 dark:border-gray-700 mb-8 print:mb-6" />
                <Section title="📍 Places" count={destinations.length}>
                  <div className="space-y-6 print:space-y-4">
                    {destinations.map((dest) => (
                      <div key={dest.id} className="pl-4 border-l-2 border-gray-200 dark:border-gray-700 print:break-inside-avoid">
                        <div className="flex flex-wrap items-baseline gap-x-2 mb-1">
                          <span className="font-semibold text-gray-900 dark:text-white">{dest.name}</span>
                          <span className="text-sm text-gray-400 dark:text-gray-500">{dest.country}</span>
                        ...
                        {dest.notes && (
                          <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">{dest.notes}</p>
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-8 print:space-y-6">
                    {memories.map((memory) => (
                      <div key={memory.id} className="print:break-inside-avoid">
                        {/* Meta */}
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-400 dark:text-gray-500 mb-1.5">
                          {memory.memoryDate && (
                        ...
                        {/* Title */}
                        <h3 className="font-bold text-gray-900 dark:text-white text-[15px] mb-2">{memory.title}</h3>

                        {/* Journal */}
                        {memory.journalEntry && (
                          <p className="text-[13.5px] text-gray-700 dark:text-gray-200 leading-[1.8] mb-3 whitespace-pre-wrap">
                            {memory.journalEntry}
                          </p>
                        )}
                        ...
                        {/* Dashed separator */}
                        <div className="mt-6 border-b border-dashed border-gray-200 dark:border-gray-700 print:mt-4" />
                      </div>
                    ))}
                  </div>
                </Section>
            ...
            {/* Footer */}
            <div className="mt-12 pt-5 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between text-xs text-gray-300 dark:text-gray-600 print:mt-8">
